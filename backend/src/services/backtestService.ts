...
import { runBacktest, BacktestSignal, BacktestTrade as BT, BacktestMetrics } from "../analysis/backtest.js";
...
export async function runBacktestForRange(
  fromDate: string,
  toDate: string,
  mode: "INTRADAY" | "SWING",
  label?: string
  ...
  const setups = await prisma.tradeSetup.findMany({
    where: {
      ...
      status: { in: TRADING_STATUSES as any },
      entryLow: { not: null },
      stopLoss: { not: null },
    },
    include: { stock: true },
  ...
  const signals: BacktestSignal[] = setups.map((s) => ({
    ...
    marketCondition: (s.marketCondition as any) ?? null,
...
export async function getBacktestRun(id: number) {
