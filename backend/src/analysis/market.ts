...
export interface MarketInput {
  indexSubset: { return20d: number | null; return5d: number | null; above20dma: boolean | null }[] | null;
  advancers: number;
  decliners: number;
  week52High: number;
  week52Low: number;
...
export function classifyMarket(input: MarketInput): {
  ...
  if (input.indexSubset && input.indexSubset.length > 0) {
    ...
    if (valid.length > 0) {
      ...
      if (avgR20 > 0 && avgR5 >= 0 && ratio >= 0.5) condition = "BULLISH";
      else if (avgR20 < 0 && avgR5 <= 0 && ratio <= 0.5) condition = "BEARISH";
      return {
        condition,
