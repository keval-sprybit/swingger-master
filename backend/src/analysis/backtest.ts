...
import { PriceBar } from "./technical.js";

export interface BacktestSignal {
  signalDate: Date;
  symbol: string;
  mode: "INTRADAY" | "SWING";
  classification: "A_PLUS" | "A" | "B" | "C" | "D" | null;
  score: number | null;
...
export interface BacktestTrade {
  ...
  exitReason: "TARGET1" | "STOP" | "TIME" | null;
...
export function runBacktest(
  ...
  for (const sig of signals) {
    ...
    let exitReason: BacktestTrade["exitReason"] = null;
...
function computeMetrics(trades: BacktestTrade[]): BacktestMetrics {
  ...
  const rrList = trades.map((t) => (t.entry - t.stop) > 0 ? Math.abs((t.target ?? t.entry) - t.entry) / (t.entry - t.stop) : null).filter((x): x is number => x != null);
  const holding = trades.map((t) => t.holdingDays).filter((x): x is number => x != null);

  const equity = [0];
  for (const t of closed) equity.push(equity[equity.length - 1] + (t.pnlPct ?? 0));
  let maxDd = 0;
  let peak = 0;
