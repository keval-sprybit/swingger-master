...
import { TradeSetupResult, MetricInput } from "./types.js";
import { TechnicalIndicators, TrendClass } from "./technical.js";
...
export interface ExplainableScore {
  ...
  why: string[]; // "why this stock" bullet list
  warnings: string[];
...
export interface ScoringInput {
  ...
  setup: TradeSetupResult | null;
  persistence?: { count: number; total: number }; // intraday signal persistence
  dataQuality?: number; // 0..1 fraction of required data available
}

const clamp01 = (x: number) => Math.max(0, Math.min(1, x));

function volumeStrength(m: MetricInput, tech: TechnicalIndicators | null): { score: number; max: number; reason: string } {
  let score = 0;
  ...
  if (ratio != null) {
    ...
    if (m.isVolumeGainer) reasons.push("Classified as Volume Gainer");
    if (tech?.relVolume != null && tech.relVolume >= 2) reasons.push(`Relative volume ${tech.relVolume.toFixed(1)}x`);
  } else {
...
function momentumScoreFn(m: MetricInput): { score: number; max: number; reason: string } {
  const chg = m.changePercent;
  let score = 0;
  if (chg != null) {
    if (chg >= 8) score = 15;
    else if (chg >= 5) score = 13;
    else if (chg >= 3) score = 11;
    else if (chg >= 2) score = 9;
    else if (chg >= 1) score = 6;
    else if (chg >= 0) score = 3;
    else if (chg <= -8) score = 2;
  }
  const reasons: string[] = [];
  if (chg != null) reasons.push(`1-day change ${chg >= 0 ? "+" : ""}${chg.toFixed(2)}%`);
  if (m.isTopGainer) reasons.push("Top Gainer");
  if (m.isTopLoser) reasons.push("Top Loser");
  return { score, max: 15, reason: reasons.join(" · ") || "No momentum data" };
}

function technicalTrendComponent(tech: TechnicalIndicators | null): { score: number; max: number; reason: string } {
  if (!tech || !tech.trend) return { score: 3, max: 15, reason: "Insufficient history for trend" };
  const map: Record<TrendClass, number> = {
    STRONG_BULLISH: 15,
    BULLISH: 13,
    NEUTRAL: 7,
    BEARISH: 4,
...
function breakoutComponent(m: MetricInput, setup: TradeSetupResult | null): { score: number; max: number; reason: string } {
  let score = 0;
  const reasons: string[] = [];
  if (setup?.breakoutLevel != null) {
    reasons.push(`Breakout level ${setup.breakoutLevel.toFixed(2)}`);
    if (setup.breakoutReason) reasons.push(setup.breakoutReason);
  }
  if (m.is52wHigh) {
    score += 8;
  ...
  const proximity =
    setup?.breakoutLevel != null && setup.currentPrice
...
function relativeVolumeComponent(tech: TechnicalIndicators | null, m: MetricInput): { score: number; max: number; reason: string } {
  const rv = tech?.relVolume ?? null;
  let score = 0;
  if (rv != null) {
    if (rv >= 5) score = 10;
    else if (rv >= 3) score = 8;
    else if (rv >= 2) score = 6;
  ...
  if (m.volumeRatio1w != null) {
    ...
    else if (r >= 1.5) score = 2;
    return { score, max: 10, reason: `Report volume ratio ${r.toFixed(1)}x (fallback)` };
  }
  return { score, max: 10, reason: "No relative-volume data" };
}

function persistenceComponent(persistence?: { count: number; total: number }): { score: number; max: number; reason: string } {
...
function marketComponent(marketCondition: "BULLISH" | "NEUTRAL" | "BEARISH" | null): { score: number; max: number; reason: string } {
  if (!marketCondition) return { score: 2, max: 5, reason: "No market data" };
  const map = { BULLISH: 5, NEUTRAL: 3, BEARISH: 1 } as const;
  return { score: map[marketCondition], max: 5, reason: `Market ${marketCondition}` };
}

function riskRewardComponent(setup: TradeSetupResult | null, max: number): { score: number; max: number; reason: string } {
  if (!setup || setup.riskReward1 == null) return { score: 0, max, reason: "No risk/reward computed" };
  const rr = Number(setup.riskReward1);
  let score = 0;
  if (rr >= 3) score = max;
  else if (rr >= 2) score = Math.round(max * 0.85);
  else if (rr >= 1.5) score = Math.round(max * 0.6);
  else if (rr >= 1) score = Math.round(max * 0.35);
  else score = 0;
  return { score, max, reason: `R:R 1:${rr.toFixed(1)}` };
}

function priceStructureComponent(tech: TechnicalIndicators | null): { score: number; max: number; reason: string } {
  const max = 10;
  if (!tech || !tech.structure.last20) return { score: 0, max, reason: "Insufficient structure data" };
  let score = 0;
  const reasons: string[] = [];
...
function dataQualityComponent(dq?: number): { score: number; max: number; reason: string } {
  const v = dq ?? 0;
  return { score: Math.round(v * 5 * 10) / 10, max: 5, reason: `Data quality ${Math.round(v * 100)}%` };
}

export function computeExplainableScore(input: ScoringInput): ExplainableScore {
  ...
  if (mode === "INTRADAY") {
    components.push(
      { key: "volume_strength", label: "Volume Strength", ...vs },
      { key: "price_momentum", label: "Price Momentum", ...mom },
      { key: "signal_persistence", label: "Signal Persistence", ...pers },
      { key: "technical_trend", label: "Technical Trend", ...trend },
      { key: "breakout_setup", label: "Breakout Setup", ...bo },
      { key: "relative_volume", label: "Relative Volume", ...rv },
      { key: "market_condition", label: "Market Condition", ...mk },
      { key: "risk_reward", label: "Risk/Reward", ...rr }
    );
  } else {
    components.push(
      { key: "trend", label: "Trend", ...trend },
      { key: "price_momentum", label: "Price Momentum", ...mom },
      { key: "volume_strength", label: "Volume Strength", ...vs },
      { key: "breakout_setup", label: "Breakout / Resistance Setup", ...bo },
      { key: "price_structure", label: "Historical Price Structure", ...ps },
      { key: "intraday_persistence", label: "Intraday Persistence", ...pers },
      { key: "market_condition", label: "Market Condition", ...mk },
      { key: "risk_reward", label: "Risk/Reward", ...rr },
      { key: "data_quality", label: "Data Quality", ...dq }
    );
  }
  ...
  for (const c of components) {
    if (c.score >= c.max * 0.7 && c.max > 0) why.push(c.reason);
  }
  if (metric.is52wHigh) why.push("Making new 52-week highs");
  if (metric.isVolumeGainer) why.push("Unusual volume expansion");
  if (tech?.relVolume != null && tech.relVolume >= 1.5) why.push("Strong relative volume");
  if (tech?.sma20 != null && metric.ltp != null && metric.ltp > tech.sma20) why.push("Above 20 DMA");
  if (tech?.sma50 != null && metric.ltp != null && metric.ltp > tech.sma50) why.push("Above 50 DMA");
  ...
  if (persistence && persistence.count >= 2) why.push(`Intraday signal persistence ${persistence.count}/${persistence.total}`);
  ...
  if (metric.is52wLow) warnings.push("New 52-week low — not a long candidate.");
  if (marketCondition === "BEARISH") warnings.push("Bearish market — require stronger setups and smaller size.");
  if (metric.ltp != null && metric.ltp < 10) warnings.push("Low-priced stock — higher risk / lower liquidity premium.");
