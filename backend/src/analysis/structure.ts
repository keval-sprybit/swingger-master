...
import { MetricInput, ScoreResult, SetupSettings } from "./types.js";
...
export function computeStructure(input: StructureInput): StructureDecision {
  ...
  if (resistanceAbove?.level != null && resistanceAbove.level > entryHigh) {
    ...
    const cand = [atrExt, swing].filter((x): x is number => x != null && x > entryHigh).sort((a, b) => a - b);
  ...
  if (!aboveBreakout) {
    if (distPct <= 3) breakoutStatus = "BREAKOUT APPROACHING";
    else breakoutStatus = "WAIT BREAKOUT";
  } else {
  ...
  if (score.classification === "D" && rr1 < 1) {
    ...
    if (status === "AVOID") reasons.push("Bearish market and setup not strong enough.");
    else reasons.push("Bearish market — watch only, require confirmation.");
...
function metricsPassConfirmation(metric: MetricInput, tech: TechnicalIndicators | null): boolean {
  ...
  if (rv != null && rv < 1.2) return false; // no volume confirmation
  if (metric.closePosition != null && metric.closePosition < 0.5) return false; // closed weak
  if (metric.is52wLow) return false;
  return true;
}
...
function buildWhy(
  ...
  const why: string[] = [];
  if (metric.isVolumeGainer || (tech?.relVolume != null && tech.relVolume >= 1.5)) why.push("Strong relative volume");
  if (metric.isTopGainer && metric.changePercent != null && metric.changePercent > 0) why.push("Positive momentum (top gainer)");
  ...
  if (metric.is52wLow) why.push("New 52-week low — avoid longs");
  if (breakoutStatus && breakoutStatus.includes("CONFIRMED")) why.push("Breakout confirmed");
  if (status === "ENTRY_ACTIVE") why.push("Entry active — confirmed break with volume");
  if (rr1 >= 2) why.push(`Good risk/reward (1:${rr1.toFixed(1)})`);
  if (tech?.trend === "BULLISH" || tech?.trend === "STRONG_BULLISH") why.push("Bullish trend structure");
  return why;
}
