...
import {
  ...
  createPaperTrade,
  updatePaperTrade,
  computePaperTradeStats,
  getPaperTrade,
} from "../repositories/paperTrades.js";
...
export const paperTradeController = {
  ...
  create: asyncHandler(async (req: any, res: Response) => {
    ...
    if (!parsed.success) return res.status(400).json({ error: "Invalid paper trade", detail: parsed.error.message });
    const d = parsed.data;
  ...
  get: asyncHandler(async (req: any, res: Response) => {
