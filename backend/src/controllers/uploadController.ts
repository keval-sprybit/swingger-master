...
export const uploadController = {
  uploadFiles: asyncHandler(async (req: any, res: Response) => {
    const files = req.files as Express.Multer.File[] | undefined;
    if (!files || files.length === 0) {
      return res.status(400).json({ error: "No files provided." });
    }
    const reportType = (req.body?.reportType as string) || undefined;
    const analysisType = (req.body?.analysisType as "EOD" | "INTRADAY") || "EOD";
    const tradingDate = (req.body?.tradingDate as string) || undefined;
  ...
  preview: asyncHandler(async (req: any, res: Response) => {
  ...
  list: asyncHandler(async (_req: any, res: Response) => {
