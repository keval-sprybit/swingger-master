...
export async function loadPriceBars(stockId, limit = 260) {
...
export async function loadPriceBarsForStockIds(stockIds, limit = 260) {
    const bars = await prisma.dailyPriceBar.findMany({
...
export async function loadPriceBarsForSymbol(symbol, limit = 260) {
...
export async function loadPriceBarsForSymbols(symbols, limit = 300) {
...
export async function coverageForStock(stockId) {
    const bars = await prisma.dailyPriceBar.findMany({
        where: { stockId },
        orderBy: { tradingDate: "asc" },
    ...
    if (bars.length === 0)
        return { available: false, availableDays: 0, missingDates: 0, fromDate: null, toDate: null };
