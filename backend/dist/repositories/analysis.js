...
export async function finalizeAnalysisRun(id, data) {
...
export async function clearRunOutputs(runId) {
...
export async function insertSetup(params) {
    ...
    return prisma.tradeSetup.create({
        data: {
            ...
            trend: st.trend,
            trendReasons: (st.trendReasons ?? null),
            marketCondition: st.marketCondition ?? null,
            whySelected: (st.whySelected ?? null),
            insufficientData: st.insufficientData ?? false,
            explainableScore: params.explainable?.total ?? null,
            explainableJson: params.explainable ? params.explainable : null,
            technicalContext: (params.technicalContext ?? null),
        },
    });
}
