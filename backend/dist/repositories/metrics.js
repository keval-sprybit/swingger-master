...
export async function ensureTradingDay(tradingDate, status = MarketStatus.CLOSED) {
...
export async function rebuildDailyMetrics(tradingDate) {
    ...
    for (const r of maVal) {
        const d = get(r.stockId, r.symbol);
        if (d.ltp === null || d.ltp === undefined)
            d.ltp = n(r.ltp);
        if (d.turnover === null || d.turnover === undefined)
            d.turnover = n(r.turnover);
        if (d.highPrice === null)
            d.highPrice = n(r.highPrice);
        if (d.lowPrice === null)
    ...
    for (const r of gainers) {
        ...
        if (d.ltp === null)
            d.ltp = n(r.todayLtp);
        if (d.changePercent === null)
            d.changePercent = n(r.todayChangePercent);
        if (d.turnover === null)
            d.turnover = n(r.todayTurnover);
    ...
    for (const r of w52h) {
        ...
        if (d.changePercent === null)
            d.changePercent = n(r.changePercent);
        if (d.highPrice === null && r.new52wHigh)
            d.highPrice = n(r.new52wHigh);
        d.is52wHigh = true;
    }
    for (const r of w52l) {
        const d = get(r.stockId, r.symbol);
        ...
        if (d.changePercent === null)
            d.changePercent = n(r.changePercent);
        if (d.lowPrice === null && r.new52wLow)
            d.lowPrice = n(r.new52wLow);
        d.is52wLow = true;
    }
    for (const r of topG) {
        const d = get(r.stockId, r.symbol);
    ...
    for (const r of large) {
        ...
        if (r.buySell === "BUY")
            d.bulkBuyQuantity = (d.bulkBuyQuantity ?? 0n) + q;
        else if (r.buySell === "SELL")
            d.bulkSellQuantity = (d.bulkSellQuantity ?? 0n) + q;
    }
    ...
    for (const d of drafts.values()) {
        ...
        const sourceCount = (d.isMostActiveVolume ? 1 : 0) +
