...
export async function createUpload(input) {
    const metadata = {};
    if (input.detectedColumns?.length)
        metadata.columns = input.detectedColumns;
    if (input.reusedFromUploadId !== undefined)
        metadata.reusedFrom = input.reusedFromUploadId;
...
export async function findUploadByChecksum(checksum) {
...
export function resolveDataUploadId(upload) {
    if (!upload)
        return undefined;
...
export async function createReuseUpload(source, opts) {
...
export function toDataUploadIds(uploads) {
    return uploads.map((u) => resolveDataUploadId(u)).filter((id) => id !== undefined);
}
export async function countUploadsForDate(tradingDate, reportType) {
...
export async function latestUploadPerReportType(tradingDate, opts) {
    ...
    for (const u of uploads) {
        if ((maxVersion[u.reportType] ?? 0) < u.uploadVersion)
            maxVersion[u.reportType] = u.uploadVersion;
    }
    ...
    const result = [];
    for (const u of uploads) {
        if (u.uploadVersion !== targetVersion)
            continue;
        ...
        if (dataId !== undefined && !entry.uploadIds.includes(dataId))
            entry.uploadIds.push(dataId);
    }
    return result;
}
...
export async function listSnapshotsForDate(tradingDate) {
    const uploads = await prisma.csvUpload.findMany({
