...
export function classifyTrend(closes, sma20, sma50, sma200) {
    const reasons = [];
    if (closes.length < 1)
        return { trend: null, reasons };
...
export function computeIndicators(bars) {
    ...
    const avgVolume20 = avgVolume20Arr.length >= 20
    ...
    const structure = {
        recent: recent.length >= 5
            ...
            : null,
        last20: struct20.high && struct20.low
            ? {
            ...
            : null,
        last50: struct50.high && struct50.low
            ? {
