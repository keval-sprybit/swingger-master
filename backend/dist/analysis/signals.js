...
function volumeExpansionScore(ratio) {
...
export function computeSignals(m) {
    const signals = [];
    const warnings = [];
    ...
    if (m.is52wHigh) {
        week52Score = 15;
        if (m.isMostActiveVolume || m.isMostActiveValue || m.isVolumeGainer)
            week52Score += 5;
    ...
    if (m.is52wLow)
        warnings.push("New 52-week low - avoid treating as a buy.");
    if (chg > 12)
        warnings.push("High one-day movement - possible volatility/chase risk.");
    const rawScore = activityVolumeScore +
        activityValueScore +
