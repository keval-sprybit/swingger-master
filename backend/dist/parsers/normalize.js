...
export function normalizeOhlc(row, colMap) {
...
export function normalizeVolumeGainer(row, colMap) {
    const symbol = (get(row, colMap, "symbol") ?? "").toUpperCase().trim();
...
export function normalizeWeek52High(row, colMap) {
    const symbol = (get(row, colMap, "symbol") ?? "").toUpperCase().trim();
    return {
        symbol,
...
export function normalizeWeek52Low(row, colMap) {
    const symbol = (get(row, colMap, "symbol") ?? "").toUpperCase().trim();
    return {
        symbol,
        series: get(row, colMap, "series"),
...
export function normalizeLargeDeal(row, colMap, filenameDate) {
    const symbol = (get(row, colMap, "symbol") ?? "").toUpperCase().trim();
...
export function normalizeBhavcopy(row, colMap) {
    const symbol = (get(row, colMap, "symbol") ?? "").toUpperCase().trim();
    ...
    const changePercent = prevClose != null && prevClose > 0 && closePrice != null
