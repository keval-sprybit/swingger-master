...
import { REPORT_SCHEMAS, detectReportType, detectTradingDate, buildColumnMap, } from "./columns.js";
import { normalizeOhlc, normalizeVolumeGainer, normalizeWeek52High, normalizeWeek52Low, normalizeLargeDeal, normalizeBhavcopy, } from "./normalize.js";
export function detectAndParse(buffer, originalFilename) {
    ...
    if (schema && !detection.needsReview) {
        ...
        for (const row of rows) {
            ...
            if (nr.symbol && nr.symbol.length > 0) {
                ...
                invalidRows++;
                if (invalidRows <= 5)
                    errors.push(`Row missing symbol: ${JSON.stringify(row).slice(0, 120)}`);
            }
        }
    }
        ...
        errors.push(`Could not confidently detect report type. Candidates: ${detection.candidates.join(", ") || "none"}.`);
    }
    return {
        ...
        dealDatesDetected: detection.reportType === "LARGE_DEALS" &&
