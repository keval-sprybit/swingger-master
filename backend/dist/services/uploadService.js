...
function toReportType(s) {
    return REPORT_TYPES.includes(s) ? s : null;
}
export async function processUpload(buffer, originalFilename, opts = {}) {
    ...
    if (opts.tradingDate) {
        ...
        if (isNaN(forced.getTime()))
            return { status: "FAILED", errors: ["Invalid forced trading date."] };
        tradingDate = forced;
    }
    ...
    const analysisType = opts.analysisType === "INTRADAY" ? AnalysisType.INTRADAY : AnalysisType.EOD;
